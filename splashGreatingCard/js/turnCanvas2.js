    ...
    (function () {
        ...
        PocketProfile.prototype = {
            drawProfile: function () {
                ...
                proctx.drawImage(this.src, -85, -110);//红包头像宽170，高220
                proctx.restore();
            }
        };
            ...
            (function (ind) {
                ...
                image.onload = function () {
                    ...
                    if (imageLoadProFinishCount == packetProfileSrc.length) {
                        ...
                        pocketProfileArray[0].alpha = 1.5;
        ...
        function rotateCircle(isClockwise, currentIndex, pindex) {
            ...
            pointArray.forEach(function (point) {
                if (isClockwise) {
                    point.rotateAngle = Math.tween.Circ.easeIn(0,point.rotateAngle,-Math.PI/6,100);
                } else {
                    point.rotateAngle = Math.tween.Circ.easeIn(0,point.rotateAngle,Math.PI/6,100);
                }
                point.drawPoint(ctx);
            ...
            if (isClockwise) {
                pointArray[currentIndex].scaleRatio = Math.tween.Circ.easeIn(0,0,0.8,100);
                pointArray[currentIndex - 1 < 0 ? 11 : currentIndex - 1].scaleRatio = Math.tween.Circ.easeIn(0,0.8,-0.8,100);
            } else {
                pointArray[currentIndex].scaleRatio = Math.tween.Circ.easeIn(0,0,0.8,100);
                pointArray[currentIndex + 1 >= 12 ? 0 : currentIndex + 1].scaleRatio = Math.tween.Circ.easeIn(0,0.8,-0.8,100);
            }
            pointArray[currentIndex].drawPoint(ctx, '#fee94e');

            profileArray.forEach(function (profile) {
                if (isClockwise) {
                    profile.rotateAngle = Math.tween.Circ.easeIn(0,profile.rotateAngle,-Math.PI/6,100);
                    profile.selfAngle = Math.tween.Circ.easeIn(0,profile.rotateAngle,-Math.PI/6,100);
                } else {
                    profile.rotateAngle = Math.tween.Circ.easeIn(0,profile.rotateAngle,Math.PI/6,100);
                    profile.selfAngle = Math.tween.Circ.easeIn(0,profile.rotateAngle,Math.PI/6,100);
                }
            ...
            if (isClockwise) {
                profileArray[currentIndex].scaleRatio = Math.tween.Circ.easeIn(0,0,0.8,100);
                profileArray[currentIndex - 1 < 0 ? 11 : currentIndex - 1].scaleRatio = Math.tween.Circ.easeIn(0,0.8,-0.8,100);
            } else {
                profileArray[currentIndex].scaleRatio = Math.tween.Circ.easeIn(0,0,0.8,100);
                profileArray[currentIndex + 1 >= 12 ? 0 : currentIndex + 1].scaleRatio = Math.tween.Circ.easeIn(0,0.8,-0.8,100);
            }
            profileArray[currentIndex].drawImg(ctx);

            if (isClockwise) {
                pocketProfileArray[pindex].alpha = Math.tween.Circ.easeIn(0,0,1,100);
                pocketProfileArray[pindex - 1 < 0 ? 5 : pindex - 1].alpha = Math.tween.Circ.easeIn(0,1,-1,100);
            } else {
                pocketProfileArray[pindex].alpha = Math.tween.Circ.easeIn(0,0,1,100);
                pocketProfileArray[pindex + 1 >= 6 ? 0 : pindex + 1].alpha = Math.tween.Circ.easeIn(0,1,-1,100);
            }
